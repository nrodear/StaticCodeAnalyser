# SCA131 — IfThen() évalue les deux branches - pas de court-circuit

![Severity](https://img.shields.io/badge/severity-Warning-orange) ![Type](https://img.shields.io/badge/type-Bug-red)

> `Math.IfThen(cond, A(), B())` et `StrUtils.IfThen` sont des fonctions ordinaires - les deux bras s'exécutent avant l'appel

## Métadonnées

| Champ | Valeur |
|---|---|
| **Identifiant de règle** | `SCA131` |
| **Kind** | `IfThenShortCircuit` |
| **Sévérité par défaut** | Warning |
| **Type** | Bug |
| **Unité du détecteur** | `uIfThenShortCircuit.pas` |
| **Étiquettes** | `control-flow`, `side-effects`, `performance`, `sonardelphi` |
