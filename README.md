...
| 📊 **Sonar-style dashboard** | Stat tiles above the grid: Errors / Warnings / Hints / Bugs / Vulnerabilities / Code Quality as a letter grade **A–E** (Sonar-style; raw score + breakdown in the tooltip) |
...
| **Analyse a project outside Delphi** (RAD not installed / batch machine) | — | ✅ pick a folder, click **▶ Analyse** | ✅ `analyser.exe <folder>` |
...
1. **Install** the plugin — three routes, in recommended order:
   - **Setup EXE** (recommended): download
     `StaticCodeAnalyserSetup-<Version>.exe` from the GitHub releases and
     run it. Per-user install, no admin rights required.

     The setup asks **which IDEs** to install into and lists only the ones
     it actually finds on the machine:

     | | installs to | registers under |
     |---|---|---|
     ...
     Only the ticked entries are installed, and unticking a previously
     installed one removes it — file and registration. The setup also
     verifies that no Delphi IDE is running, cleans up conflicting
     registrations of the development package set, offers a
     German/English language choice, shows the MIT license page and
     creates a Start-menu uninstall entry.
   - **Build it yourself**: open
     ...
     compiles but never appears in the IDE menu. Full instructions in
     [HowTo_Build.md](HowTo_Build.md).
   - **GetIt local package**: download the release asset
     `StaticCodeAnalyser-v<V>-getit-manifest.zip`, unpack it, then in the
     GetIt Package Manager choose **Load Local Package** (bottom of the
     dialog; requires an active Update Subscription). The payload
     `...-plugin-getit.zip` is fetched by GetIt itself from the URL inside
     the manifest — both are attached to the release. This route installs
     the Delphi 12 plugin.
2. In Delphi: **View → Static Code Analysis Tool for Delphi** — the
   dockable window shows up.
3. Pick a project path → click **▶ Analyse**.
...
Every release on the [releases page](https://github.com/nrodear/StaticCodeAnalyser/releases)
ships five artifacts:

| Artifact | What it is |
|---|---|
| `StaticCodeAnalyserSetup-<V>.exe` | **IDE plugin installer.** Per-user, no admin rights. Picks the target IDEs from what it finds — Delphi 12 (32-bit) and Delphi 13 (32-bit and 64-bit). |
| `StaticCodeAnalyser-v<V>-Win32.zip` | **Standalone EXE, 32-bit**, with the rule catalogue. Stack reserve patched to 32 MB — needed for deep ASTs on large code bases. |
| `StaticCodeAnalyser-v<V>-Win64.zip` | **Standalone EXE, 64-bit.** Use this one for large corpora; the address space is the limit that matters there. |
| `StaticCodeAnalyser-v<V>-source.zip` | **Sources** as of the release tag. |
| `StaticCodeAnalyser-v<V>-docs-scripts.zip` | **Documentation and CI examples** — all guides in English, German and French, plus `examples/` (GitHub-Actions workflow with SARIF upload, pre-commit hook, PR comment bot, MSBuild target, rule profiles). |

The plugin and the standalone EXE share the same engine and the same
rule catalogue; they differ only in how you drive them.
...
| **▶ Analyse** | Recursive scan — what is analysed (folder / project / group) is shown in the button hint |
| **📄 File** | Just the `.pas` file currently open in the editor |
...
**▶ Analyse** again. Settings are reloaded on every run, no IDE
restart required.
...
A **Quality score tile** next to them shows the code quality as a letter
grade **A–E** (Sonar-style); the raw score and its breakdown appear in
the tile's tooltip.
...
- **Profile dropdown**: switch the active rule-set on the fly. Bundled
  ...
  (everything except the seven pure-convention rules), `strict` (really
  everything), `style` (exactly those seven), `security` (vulns + hotspots
...
The analyser scans Lazarus projects natively — same engine, same
rules, dialect-aware:

| What | How |
|---|---|
| **Dialect switch** | `--dialect fpc` — additionally collects `*.pp` and `*.lpr` units, pairs `.lfm` form files, parses the one-branch view with the FPC/LCL defines, and disables the three Delphi-only rules below. `--dialect auto` resolves per file (see the IFDEF section above). |
| **Project scan** | `--project` accepts `.lpi` / `.lpk` (and `--project-group` accepts `.lpg`) and **forces the fpc dialect** for that run. |
| **Default without switch** | `[Scan] Dialect=delphi\|fpc` in `analyser.ini` (the switch always wins). The GUI reads the same key and offers a toggle via **right-click on the status bar**; an active fpc dialect shows as `Dialect=fpc` in the rule-set indicator. |
| **Delphi-only rules, skipped under fpc** | `SCA041` (VCL layer model does not apply to LCL), `SCA129` (FPC `string` is 8-bit/UTF-8 — the cast is not a lossy narrowing), `SCA185` (BOM-less UTF-8 is the Lazarus **norm**; invalid-UTF-8 and ANSI checks stay active). Details on each rule page. |
| **IDE plugin** | Delphi-IDE only (by design) — Lazarus users scan via CLI or the standalone GUI. |
...
seven pure-convention rules** — public-member docs, `Assigned()` vs `<> nil`,
`begin..end` on single-statement branches, asymmetric `begin..end` around
`if`/`else`, `with`, one-class-per-unit and hardcoded DFM captions. They
are correct findings, but they are conventions,
and together they were 48 % of everything a newcomer saw. `--profile style`
...
Clicking **📄 File** in the IDE plugin activates a single-file live watch
...
tabs to another file changes nothing; clicking **📄 File** again moves the
watch to the new file. Bulk paths (**▶ Analyse**, **Branch changes**)
...
```
StaticCodeAnalyserIDE/                 IDE expert packages: dev set
                                       (SCA.Engine.dpk + SCA.SharedUI.dpk +
                                       StaticCodeAnalyser.IDE.d12.dpk) and
                                       release monolith
...
| DFM parser + 23 DFM detectors (per `.dfm`) | ~5–20 ms | ~5–10 s |
...
- **DFMCheck / GExperts DFM-Check** — single-purpose DFM linters. The
  23 DFM detectors in this project are a superset (graph-based
