...
| **Phase 2.1** | Sibling-Write-Check für if-then-else mit beidseitigem Write | 4-6h | obsolet — pessimistic-Write registriert sowieso jeden Write-Ast, kein Confidence-Downgrade-Pfad existiert. Würde erst relevant mit echtem CFG (Phase 3). | ⏭️ **obsolet** |
...
| **Phase 2.6** | Source-Line-based Nested-Method-Detection als robuster Ersatz für Phase 2.4 (Parser entfernt Outer-MethodNode bei 'Headless-Method'-Pattern → AST-Walk findet nested nicht). Source-Scan nach `^\s{2,}(procedure\|function\|...)` + begin/end-Pair-Counting | 2h | partial — −7 Findings (996 → 989). begin/end-Counter wird durch inline-`if-then-begin-end`-Pattern verwirrt; `uConsoleRunner.pas:142` bleibt FP. Robustere Source-Heuristik bräuchte echten Pascal-Mini-Lexer | 🟡 commit c86aafb |
