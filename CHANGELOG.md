...
- **`--report-csv` and `--report-json`.** Both writers existed, but only
  ...
  scripted. They honour `--base-dir` like every other CLI export, so a CI
  artifact does not carry machine-specific absolute paths. The JSON
  export lost its UTF-8 BOM in the process, for the same reason as the
  other JSON outputs; the CSV keeps it, because that is how Excel
  recognises UTF-8.
